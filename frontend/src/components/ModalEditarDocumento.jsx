...
const ModalEditarDocumento = ({ documento, onClose, onSave }) => {
  const [setores, setSetores] = useState([]);
  const [formData, setFormData] = useState({ ...documento });
  const [loading, setLoading] = useState(false);
  const [renovacoes, setRenovacoes] = useState([]);
  const [novaRenovacao, setNovaRenovacao] = useState({ descricao: '', nova_data_vigencia: '', pdf: null });
  const [loadingRenovacao, setLoadingRenovacao] = useState(false);
  const [editingRenovacaoId, setEditingRenovacaoId] = useState(null);
  const [editRenovacaoData, setEditRenovacaoData] = useState({ descricao: '', nova_data_vigencia: '', pdf: null });
  ...
  useEffect(() => {
    ...
    setFormData({
      ...documento,
      data_emissao: formatForInput(documento.data_emissao),
      data_vigencia_fim: formatForInput(documento.data_vigencia_fim),
      dias_alerta: documento.dias_alerta || 30,
      status: documento.status || 'VIGENTE',
  ...
  const fetchRenovacoes = async () => {
    if (!documento) return;
    try {
      const res = await axios.get(`/api/documentos/${documento.id}/renovacoes`);
      setRenovacoes(res.data);
    } catch (err) {
      console.error(err);
    }
  };
    ...
    <div style={overlayStyle}>
      <div className="glass-card" style={modalStyle}>
        <div className="flex-between" style={{ marginBottom: '24px' }}>
          <h2 style={{ fontSize: '1.4rem', margin: 0 }}>Editar Documento</h2>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <select name="tipo" required value={formData.tipo} onChange={handleChange} style={inputStyle}>
                <option value="CND">Certidão Negativa de Débitos (CND)</option>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <input type="text" name="orgao_emissor" value={formData.orgao_emissor} onChange={handleChange} style={inputStyle} />
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              {documento.anexos && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '4px' }}>
                  <a 
                    href={`/api/documentos/${documento.id}/anexo`} 
                    target="_blank" 
                  ...
                  <button 
                    ...
                    onClick={async () => {
                      ...
                      try {
                        await axios.delete(`/api/documentos/${documento.id}/anexo`);
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px', gap: '12px' }}>
            ...
            <button type="submit" disabled={loading} style={saveBtnStyle}>
              {loading ? 'Salvando...' : <><Save size={18}/> Salvar Dados do Documento</>}
            </button>
          </div>
        </form>
        ...
        {/* SECÃO DE RENOVAÇÕES */}
        <div>
          <h3 style={{ fontSize: '1.2rem', margin: '0 0 16px 0' }}>Renovações do Documento</h3>
          
          {/* Listar Renovações Existentes */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
            {renovacoes.length === 0 && (
              <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Nenhuma renovação registrada.</div>
            )}
            {renovacoes.map(r => (
              <div key={r.id} style={{ background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', border: '1px solid var(--panel-border)' }}>
                {editingRenovacaoId === r.id ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <input 
                      type="text" 
                      value={editRenovacaoData.descricao} 
                      onChange={e => setEditRenovacaoData({...editRenovacaoData, descricao: e.target.value})} 
                      style={inputStyle} 
                      placeholder="Descrição"
                    />
                    <input 
                      type="date" 
                      value={editRenovacaoData.nova_data_vigencia} 
                      onChange={e => setEditRenovacaoData({...editRenovacaoData, nova_data_vigencia: e.target.value})} 
                      style={inputStyle} 
                    />
                    <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                      <button 
                        ...
                        onClick={async () => {
                          try {
                            const payload = new FormData();
                            payload.append('descricao', editRenovacaoData.descricao);
                            payload.append('nova_data_vigencia', editRenovacaoData.nova_data_vigencia);
                            await axios.put(`/api/documentos/renovacoes/${r.id}`, payload);
                            setEditingRenovacaoId(null);
                            fetchRenovacoes();
                            onSave();
                          } catch (err) {
                      ...
                      >Salvar</button>
                      <button type="button" onClick={() => setEditingRenovacaoId(null)} style={{...cancelBtnStyle, padding: '4px 8px', fontSize: '0.85rem'}}>Cancelar</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button 
                          type="button"
                          onClick={async () => {
                            if (!window.confirm('Deseja excluir totalmente esta renovação?')) return;
                            try {
                              await axios.delete(`/api/documentos/renovacoes/${r.id}`);
                              fetchRenovacoes();
                              onSave();
                            } catch (err) {
                              alert('Erro ao excluir renovação.');
                            }
                          }}
                          style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer' }}
                          title="Excluir Renovação Inteira"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
                    ...
                    {r.anexos && (
                      <div style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <a 
                          href={`/api/documentos/renovacoes/${r.id}/anexo`}
                          target="_blank" 
                          rel="noopener noreferrer"
                        ...
                        <button 
                          type="button"
                          onClick={async () => {
                            if (!window.confirm('Excluir apenas o PDF desta renovação?')) return;
                            try {
                              await axios.delete(`/api/documentos/renovacoes/${r.id}/anexo`);
                              fetchRenovacoes();
                            } catch (err) {
                              alert('Erro ao excluir PDF da renovação.');
                            }
                          }}
                          style={{ background: 'none', border: 'none', color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', cursor: 'pointer' }}
                        >
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', padding: '16px', borderRadius: '8px', border: '1px solid var(--panel-border)' }}>
            <h4 style={{ margin: '0 0 12px 0', fontSize: '1rem' }}>Registrar Nova Renovação</h4>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={labelStyle}>Descrição *</label>
                <input 
                  type="text" 
                  value={novaRenovacao.descricao} 
                  onChange={e => setNovaRenovacao({...novaRenovacao, descricao: e.target.value})} 
                  style={inputStyle} 
                  placeholder="Ex: Renovação 2026/2027"
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                <input 
                  type="date" 
                  value={novaRenovacao.nova_data_vigencia} 
                  onChange={e => setNovaRenovacao({...novaRenovacao, nova_data_vigencia: e.target.value})} 
                  style={inputStyle} 
                />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={labelStyle}>Anexar PDF da Renovação</label>
                <input 
                  type="file" 
                  accept="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                  onChange={e => setNovaRenovacao({...novaRenovacao, pdf: e.target.files[0]})} 
                  style={inputStyle} 
                  id="renovacaoPdfInput"
                />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button 
                onClick={async () => {
                  if (!novaRenovacao.descricao || !novaRenovacao.nova_data_vigencia) {
                  ...
                  try {
                    ...
                    // Notifica a listagem pra atualizar o Documento original (vigência nova)
                    onSave();
                  } catch (err) {
                    console.error(err);
                    alert('Erro ao salvar renovação.');
                  } finally {
              ...
              >
                {loadingRenovacao ? 'Adicionando...' : <><Plus size={18}/> Adicionar Renovação</>}
              </button>
            </div>
          </div>
        </div>
